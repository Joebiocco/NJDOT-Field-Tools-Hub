...
- **Never rename/clear localStorage keys or IndexedDB stores.** Keys: `ft_bridge_bookmarks`, `ft_fuel_bookmarks`, `wo_recent`, `workorder_draft`, `ft_dc144_recent`, `ft_dc144_templates`, `ft_ts_entries`, `ft_ts_settings`, `ft_ts_ppoffset`, `field_dark_mode`, `ft_last`, `ft_*_shown`; sessionStorage `ft_opening_from_hub`/`ft_returning_to_hub`. IDB: db `ft_photos` (v2), stores `session_photos` + `dc144_sessions`.
